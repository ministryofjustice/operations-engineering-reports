...
class TestRetriesGithubRateLimitExceptionAtNextResetOnce(unittest.TestCase):
    def test_function_is_only_called_once_with_arguments(self):
        ...
        retries_github_rate_limit_exception_at_next_reset_once(mock_function)(
            mock_github_service, "test_arg"
        )
    ...
    def test_function_is_called_twice_when_transport_server_error_raised_once(self):
        mock_function = Mock(side_effect=[TransportServerError(Mock(), Mock()), Mock()])
        mock_github_client = Mock(Github)
...
class TestGithubServiceInit(unittest.TestCase):
    def test_sets_up_class(
        ...
        self.assertEqual(
            "test_mock_github_client_gql_api", github_service.github_client_gql_api
        )
        ...
        self.assertEqual(ORGANISATION_NAME, github_service.organisation_name)
        self.assertEqual(ENTERPRISE_NAME, github_service.enterprise_name)
...
class TestGithubServiceGetPaginatedListOfRepositoriesPerType(unittest.TestCase):
    ...
    def test_throws_value_error_when_page_size_greater_than_limit(
        self, _mock_gql_client
    ):
        github_service = GithubService("", ORGANISATION_NAME)
        self.assertRaises(
            ValueError,
